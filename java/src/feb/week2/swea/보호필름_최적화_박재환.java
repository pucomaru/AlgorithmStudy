...
public class 보호필름_최적화_박재환 {
    ...
    static void init() throws IOException {
         ...
         */
        if(k == 1 || isPass(film)) minInject = 0;
        else {
            findMinInject();
        }
        sb.append(minInject);
    }
    static void findMinInject() {
        /**
         * 약품 투입 횟수를 기준으로 탐색
         */
        for(int i=1; i<d+1; i++) {
            inject(0,0,i);
            if(minInject != 15) return;
        }
    }
    static void inject(int selectedId, int inj, int max) {
        if(isOk || inj >= minInject) return;

        if(inj == max) {       // 모든 경우를 탐색
            if(isPass(film)) {
                minInject = inj;
                isOk = true;
            }
        ...
        // 약품 X
        inject(selectedId+1, inj, max);

        // 약품 투입
        for (int y = 0; y < w; y++) backup[selectedId][y] = film[selectedId][y];

        // 0 투입
        Arrays.fill(film[selectedId], 0);
        inject(selectedId+1, inj+1, max);
        // 1 투입
        Arrays.fill(film[selectedId], 1);
        inject(selectedId+1, inj+1, max);

        // 원복
        for (int y = 0; y < w; y++) film[selectedId][y] =  backup[selectedId][y];
    }
